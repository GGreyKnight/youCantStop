...
public class RoadSpawner : MonoBehaviour
{
    ...
    void Start()
    {
        //spawning first road without obstacles and barier that blocks player goes back on road
        SpawnRoad(0);
    ...
    void Update()
    {
        if(carTransform != null)
        {
            if (carTransform.position.z > spawnZPosition - (numberOfRoads * roadLength))
            {
                    ...
                    {
                        moveBarierForward(barierInstance.transform);
                        DeleteRoad();
                    }
            }
        }           
    ...
    private void moveBarierForward(Transform barier)
    {
        barier.position += transform.forward * roadLength;
    }
}
